...
export const ProjectCards = () => {
  ...
  useEffect(() => {
    // Aggressive watermark removal - Polling every 100ms to ensure logo is killed immediately
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`absolute inset-[-40px] blur-[120px] rounded-full z-0 opacity-0 group-hover:opacity-100 transition-opacity duration-[2000ms] pointer-events-none ${project.glowColor === 'blue' ? 'bg-blue-600/20' :
        ...
        >
          <div className="glass-panel relative z-10 w-full h-full overflow-hidden flex flex-col rounded-2xl border border-white/5 shadow-2xl">
            <div 
              className={`absolute inset-0 z-0 transition-opacity duration-1000 will-change-opacity ${(isFlipped || !isInView) ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
            >
            ...
            <div className="relative z-10 p-12 flex flex-col justify-end bg-gradient-to-t from-black/95 via-black/40 to-transparent flex-grow">
              <span className={`text-[10px] tracking-[0.6em] font-mono mb-3 uppercase font-bold ${project.glowColor === 'blue' ? 'text-blue-400/90' :
                project.glowColor === 'green' ? 'text-green-400/90' :
                  'text-purple-400/90'
                }`}>
                {project.category || 'PROJECT'}
              </span>
              <h3 className={`text-white text-3xl md:text-4xl font-bold tracking-tighter mb-6 transition-colors duration-500 ${project.glowColor === 'blue' ? 'group-hover:text-blue-400' :
              ...
              <div className="mb-4">
                <button
                  ...
                  className={`text-xs tracking-[0.4em] uppercase font-bold transition-all duration-300 hover:tracking-[0.5em] ${project.glowColor === 'blue' ? 'text-blue-400/80 hover:text-blue-300' :
            ...
            <div className={`absolute inset-0 border transition-colors duration-1000 ${project.glowColor === 'blue' ? 'border-blue-500/10 group-hover:border-blue-500/30' :
        ...
        >
          <div className={`glass-panel relative z-10 w-full h-full p-12 flex flex-col rounded-2xl border bg-black/40 ${isMobile ? 'backdrop-blur-xl' : 'backdrop-blur-3xl'} shadow-2xl will-change-transform ${project.glowColor === 'blue' ? 'border-blue-500/30' :
            ...
            <div className="flex justify-between items-start mb-8">
              <div>
                <h4 className={`text-[10px] tracking-[0.6em] font-mono mb-2 uppercase ${project.glowColor === 'blue' ? 'text-blue-400' :
              ...
              {project.githubUrl && (
                <a
                  ...
                  className={`p-3 rounded-full border transition-all duration-300 ${project.glowColor === 'blue' ? 'border-blue-500/20 hover:bg-blue-500/20 text-blue-400' :
      ...
      <div className="w-full relative z-30 mt-4 pointer-events-auto">
        <div className={`text-[10px] tracking-[0.5em] font-mono mb-4 uppercase font-bold opacity-80 pl-4 border-l-2 ${project.glowColor === 'blue' ? 'text-blue-400 border-blue-500/30' :
